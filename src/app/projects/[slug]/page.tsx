  ...
  if (!project) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-32 text-center">
        ...
        <Link href="/projects" className="text-accent-brown hover:underline">
          Back to Projects
        </Link>
      </div>
    );
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-6 pt-8">
        ...
        >
          <Link href="/projects" className="hover:text-accent-brown transition-colors">
            Projects
          </Link>
          <span>/</span>
          <span className="text-accent-brown">{project.title}</span>
        </motion.div>
      </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-12">
        <motion.div {...fadeUp}>
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="px-3 py-1 bg-white/80 backdrop-blur-sm rounded-full text-xs font-mono text-accent-brown">
              {project.category}
            </span>
        ...
        <motion.div {...fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
          {[
            ...
            <div key={item.label} className="bg-white shadow-sm border border-accent-brown/8 rounded-xl p-4">
              <p className="text-xs font-mono text-accent-brown uppercase tracking-widest mb-2">
                {item.label}
              </p>
      ...
      <motion.div {...fadeUp} className="max-w-6xl mx-auto px-6">
        <div className="relative aspect-video rounded-xl overflow-hidden bg-white shadow-sm border border-accent-brown/8">
      ...
      <section className="max-w-3xl mx-auto px-6 py-8">
        {project.sections.map((section, i) => {
          ...
          if (section.type === "subheader") {
            return (
              <motion.h3
                key={i}
                {...fadeUp}
                className="text-lg md:text-xl font-serif font-bold text-accent-brown mt-8 mb-4"
              >
                {section.content}
              </motion.h3>
            );
          ...
          if (section.type === "bullet") {
            return (
              <motion.div key={i} {...fadeUp} className="flex gap-3 mb-3">
                <span className="text-accent-brown mt-1.5 flex-shrink-0">
          ...
          if (section.type === "numbered") {
            ...
            return (
              <motion.div key={i} {...fadeUp} className="flex gap-3 mb-3">
                <span className="text-accent-brown font-mono text-sm mt-0.5 flex-shrink-0 w-6">
                  {numberedCount}.
                </span>
                <p className="text-text-secondary leading-relaxed">{section.content}</p>
              </motion.div>
          ...
          if (section.type === "image" && section.src) {
            return (
              <motion.div key={i} {...fadeUp} className="my-8 rounded-xl overflow-hidden bg-white shadow-sm border border-accent-brown/8">
